...
using M_Manager = SAO.Controls.Moving.MoveManager;
...
namespace SAO.Controls.Elements
{
    partial class GraphicElements
    {
            ...
            if (this.WasMouseIn())
            {
                if (this.OwnerMover)
                {
                    if (this.HasOwner && this.Owner != null)
                    {
                        if (this.Owner.Movements != ElementMovements.NoMovements)
                        {
                            child.LastPoint = this.LastPoint = M_Manager.Point.ToVector2();
                            this.Owner.Shocker(this);
                        }
                    }
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        // if you reach here, it means some mistakes has been
                        // taken.
...
#if LEFT_CLICK
            var c1 = BigFather.CurrentState.LeftButton;
            var c2 = BigFather.LastMouseState.LeftButton;
            // check if c1 was in pressed status or not.
            if (c1 == ButtonState.Pressed)
            {
                // check if c2 was in released status or not.
                if (c2 == ButtonState.Released && !this.IsLeftDown)
                {
        ...
        private void _checkRightClick()
        {
            ...
            var c1 = BigFather.CurrentState.RightButton;
            var c2 = BigFather.LastMouseState.RightButton;
            // check if c1 was in pressed status or not.
            if (c1 == ButtonState.Pressed)
            {
                // check if c2 was in released status or not.
                if (c2 == ButtonState.Released && !this.IsRightDown)
                {
                    // it means the right button of the mouse was clicked.
                    this.OnRightClick();
                }
            }
#endif
        ...
        private void _checkLeftDown()
        {
            ...
            var c1 =
                BigFather.CurrentState.LeftButton;
            var c2 =
                BigFather.LastMouseState.LeftButton;
            if (c1 == ButtonState.Released)
            {
                if (c2 == ButtonState.Pressed)
                {
                    this.IsLeftDown = true;
                    this.LeftDownOnce = true;
                    this.OnLeftDown();
        ...
        private void _checkRightDown()
        {
            ...
            var c1 =
                BigFather.CurrentState.RightButton;
            var c2 =
                BigFather.LastMouseState.RightButton;
            if (c1 == ButtonState.Released)
            {
                if (c2 == ButtonState.Pressed)
                {
        ...
        /// after that, it will add itself to the manger 
        ...
        /// </param>
        /// <param name="add">
        /// if you want add this element to it's owner's manager, set this arg to 
        ...
        /// <c>false</c>.
        /// </param>
        public virtual void ChangePriority(in ElementPriority priority, in bool add = false)
        {
        ...
        /// </summary>
        /// <param name="gameTime">
        /// the <see cref="GameTime"/> of the SAO.
        /// </param>
